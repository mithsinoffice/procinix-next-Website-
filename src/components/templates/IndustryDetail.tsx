import Link from "next/link";
import { AlertCircle, ArrowUpRight } from "lucide-react";
import { Section } from "@/components/primitives/Section";
import { Card } from "@/components/primitives/Card";
import { DetailHero } from "@/components/sections/DetailHero";
import { CallToAction } from "@/components/sections/CallToAction";
import {
  MODULE_ROUTES,
  PILLAR_ROUTES,
  type RouteDef,
  type Family,
} from "@/lib/routes";
import type { IndustryContent } from "@/content/types";
import { iconForModule } from "@/lib/module-icons";

const familyMeta: Record<Family, { label: string; tone: string; pillar: RouteDef }> = {
  s2p: { label: "Source-to-Pay", tone: "var(--accent-teal)", pillar: PILLAR_ROUTES.sourceToPay },
  o2c: { label: "Order-to-Cash", tone: "var(--accent-amber)", pillar: PILLAR_ROUTES.orderToCash },
  r2r: { label: "Record-to-Report", tone: "var(--accent-purple)", pillar: PILLAR_ROUTES.recordToReport },
};

export function IndustryDetail({
  content,
  route,
}: {
  content: IndustryContent;
  route: RouteDef;
}) {
  const relevantModules = content.relevantModuleSlugs
    .map((slug) => {
      const key = Object.keys(MODULE_ROUTES).find((k) =>
        (MODULE_ROUTES as Record<string, RouteDef>)[k].path.endsWith(`/${slug}`),
      );
      return key ? (MODULE_ROUTES as Record<string, RouteDef>)[key] : null;
    })
    .filter((x): x is RouteDef => x !== null);

  return (
    <>
      <DetailHero
        eyebrow="Industry"
        eyebrowTone="teal"
        title={<>{route.label}</>}
        description={content.tagline}
        tags={content.complexityHandled.slice(0, 4)}
      />

      {/* Pain points */}
      <Section
        eyebrow="The operating reality"
        title="Complexity, not theory."
        description="The finance-operating problems we repeatedly solve in this sector."
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {content.painPoints.map((p) => (
            <Card key={p} className="p-6 flex items-start gap-4">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/[0.03] text-white/70">
                <AlertCircle className="h-4 w-4" />
              </span>
              <p className="text-[15px] text-white/80 leading-relaxed">{p}</p>
            </Card>
          ))}
        </div>
      </Section>

      {/* Use cases by finance family */}
      <Section
        eyebrow="Use cases by finance family"
        title="Where Procinix fits in your operating model."
        className="bg-[var(--bg-secondary)]/50 border-y border-white/[0.04]"
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {content.useCases.map((uc) => {
            const fm = familyMeta[uc.family];
            return (
              <Card key={uc.title} glow className="h-full p-7 flex flex-col">
                <span
                  className="text-[10.5px] uppercase tracking-[0.22em] mb-4"
                  style={{ color: fm.tone }}
                >
                  {fm.label}
                </span>
                <h3 className="font-display text-[19px] text-white leading-snug mb-3">
                  {uc.title}
                </h3>
                <p className="text-[13.5px] text-white/60 leading-relaxed mb-6 flex-1">
                  {uc.body}
                </p>
                <Link
                  href={fm.pillar.path}
                  className="inline-flex items-center gap-1.5 text-[13px] text-white/75 hover:text-white transition-colors"
                >
                  {fm.pillar.label}
                  <ArrowUpRight className="h-3.5 w-3.5" />
                </Link>
              </Card>
            );
          })}
        </div>
      </Section>

      {/* Relevant modules */}
      {relevantModules.length > 0 && (
        <Section
          eyebrow="Relevant modules"
          title="The specific modules that drive this sector."
        >
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {relevantModules.map((m) => (
              <Link
                key={m.path}
                href={m.path}
                className="group block rounded-xl border border-white/[0.07] bg-white/[0.02] p-4 hover:border-white/20 hover:bg-white/[0.04] hover:-translate-y-0.5 transition-all"
              >
                <div className="flex items-start justify-between mb-3">
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--accent-teal)]/10 text-[var(--accent-teal-bright)]">
                    {iconForModule(m.path)}
                  </span>
                  <ArrowUpRight className="h-3.5 w-3.5 text-white/25 group-hover:text-white/70 transition-colors" />
                </div>
                <div className="text-[13.5px] font-medium text-white leading-snug">
                  {m.label}
                </div>
              </Link>
            ))}
          </div>
        </Section>
      )}

      {/* Complexity handled */}
      <Section
        eyebrow="What we handle"
        title="Complexity absorbed, not simplified away."
        className="bg-[var(--bg-secondary)]/50 border-y border-white/[0.04]"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {content.complexityHandled.map((c) => (
            <div
              key={c}
              className="flex items-center gap-4 rounded-xl border border-white/[0.07] bg-white/[0.02] px-5 py-4"
            >
              <span className="h-1.5 w-1.5 rounded-full bg-[var(--accent-teal)]" />
              <span className="text-[14.5px] text-white/85">{c}</span>
            </div>
          ))}
        </div>
      </Section>

      {/* Outcomes */}
      <Section
        eyebrow="Outcomes"
        title="What this unlocks."
        description="Directional outcomes observed in operating deployments in this sector."
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {content.outcomes.map((o) => (
            <Card key={o.metric} className="p-7">
              <span className="text-[10.5px] uppercase tracking-[0.22em] text-[var(--accent-teal)] mb-4 inline-block">
                Outcome
              </span>
              <h3 className="font-display text-[20px] text-white leading-snug mb-3">
                {o.metric}
              </h3>
              <p className="text-[13.5px] text-white/60 leading-relaxed">
                {o.detail}
              </p>
            </Card>
          ))}
        </div>
      </Section>

      <CallToAction />
    </>
  );
}
